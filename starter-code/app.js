...
app.isWardenAuthenticated = function(req,res,next){
...
app.isCafeteria = function(req,res,next){
   if(req.isAuthenticated() && req.user.role !=='visitor'){
        return next()
    }
    res.redirect('/html/forbidden.html')
}

app.isNotForPrisoners = function(req,res,next){

    if(req.isAuthenticated() && req.user.role !=='prisoner'){
...
app.isCellM = function(req,res,next){
   if(req.isAuthenticated() && req.user.username ==='mallory' || req.user.role==='warden' || req.user.role==='guard'){
        return next()
    }
    res.redirect('/html/forbidden.html')
}

app.isCellE = function(req, res,next){
    if(req.isAuthenticated() && req.user.username ==='eve' || req.user.role==='warden' || req.user.role==='guard'){
        return next()
    }
    res.redirect('/html/forbidden.html')
}
...
app.get('/jail', app.isAuthenticated, function(req, res){
    console.log(`welcome`, req.user.username) // why is this a request and not a response?
    res.sendFile('/html/jail.html', {root: './public'})
})
app.get('/lobby', app.isNotForPrisoners, function(req, res){
    res.sendFile('/html/lobby.html', {root: './public'})
})
app.get('/visitors-lounge', app.isNotForPrisoners, function(req, res){
    res.sendFile('/html/visitors-lounge.html', {root: './public'})
})
app.get('/cafeteria', app.isCafeteria, function(req, res){
    res.sendFile('/html/cafeteria.html', {root: './public'})
})
app.get('/wardens-office', app.isWardenAuthenticated, function(req, res){
    res.sendFile('/html/wardens-office.html', {root: './public'})
})
app.get('/cell-e', app.isCellE, function(req, res){
    res.sendFile('/html/cell-e.html', {root: './public'})
})
app.get('/cell-m', app.isCellM, function(req, res){
